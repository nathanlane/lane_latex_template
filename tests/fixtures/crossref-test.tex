...
%% #48: the package configures cleveref but no longer loads it, so a
%% document using \cref must load hyperref and cleveref itself.
\usepackage{hyperref}
\usepackage{cleveref}
...
\item With page: \cref{fig:test} on \cpageref{fig:test}
\item Parenthetical: conclusions (\cref{tab:results})
\item See also: see also \cref{sec:intro}
\end{itemize}
