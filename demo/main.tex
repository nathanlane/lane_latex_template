...
\usepackage{lanepaper}             % Then load lanepaper
\end{verbatim}
...
\item \emph{Tables:} Standard row heights via \texttt{\textbackslash{}begin\{gridtable\}}
\item \emph{Images:} Heights rounded to quantum multiples via \texttt{\textbackslash{}gridincludegraphics}
...
\item \emph{All links:} \emph{Enhanced navy blue} (RGB 0,102,180) for consistent visual treatment
\end{itemize}
...
\safeparagraph{Grid Table Environments}~ The framework provides three table environments with standard row heights:
\begin{itemize}
\item \texttt{gridtable} --- Standard rows (~21.8pt measured) for normal data
\item \texttt{regressiontable} --- ~21.0pt rows (measured) for statistical results
\item \texttt{compactgridtable} --- ~16.9pt rows (measured) for dense information
\end{itemize}

Each environment automatically sets \texttt{\textbackslash arraystretch} to maintain the standard row height while preserving the professional appearance of booktabs.
